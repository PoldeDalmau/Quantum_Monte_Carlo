...
def mcmc_sample_6D(hops,pdf, pdfparam):                 
    ...
    for i in range(1,1+hops):                               # hops is the number of times you "hop", i.e. nr of steps...
        ...
        if random_coin(acceptance) and i % n_hopsperwalker != 0:
            ...
            #print("accepted", i, "n_acc", n_accepted)
    #print ("accepted/total =", n_accepted/hops)
    return states_x1, states_y1, states_z1, states_x2, states_y2, states_z2, n_accepted/hops, r_rp, rp_r                            # give the system some time to find a reasonable starting point. 
...
def psi_Hydrogen(x,y,z, alpha):
    ...
    ----------
    x,y,z:
        electron position in cartesian coordinates
    alpha:
...
def E_loc(x1,y1,z1,x2,y2,z2, aplha):
...
"""
    ...
    return -4 + (np.dot(r_1/m_1 - r_2/m_2, r_1 - r_2)) * 1/(r_12*(1 + alpha*r_12)**2) - 1/(r_12*(1 + alpha*r_12)**3) - 1/(4*r_12*(1 + alpha*r_12)**4) + 1/r_12
