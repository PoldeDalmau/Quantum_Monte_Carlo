...
def mcmc_sample_3D(hops,pdf, pdfparam):                 
    ...
    for i in range(hops):                               # hops is the number of times you "hop", i.e. nr of steps...
        ...
        if random_coin(acceptance) and i % n_hopsperwalker != 0:
            ...
            #print("accepted", i, "n_acc", n_accepted)
    #print ("accepted/total =", n_accepted/hops)
    return states_x[burn_in:], states_y[burn_in:], states_z[burn_in:], n_accepted/hops, r_rp[burn_in:], rp_r[burn_in:]                            # give the system some time to find a reasonable starting point. 
